...
{ // function call 3 cycles => 3C
  ...
  __asm__ __volatile__ 
    (
     ...
     " brne _Lidelay_pa01                                           ;;5C+(x-1)2C+xC\n\t"
...
void i2c_wait_scl_high_pa01(void)
{
#if I2C_TIMEOUT <= 0
  __asm__ __volatile__ 
  ...
  __asm__ __volatile__ 
    (" cbi      %[SDADDR],%[SDAPIN]     ;release SDA \n\t" 
     ...
     : :
       ...
       [SCLIN] "I" (SCL_IN_PA01), [SCLOUT] "I" (SCL_OUT_PA01),
       [SDADDR] "I"  (SDA_DDR_PA01), [SDAPIN] "I" (SDA_PIN_PA01), 
       [SDAIN] "I" (SDA_IN_PA01), [SDAOUT] "I" (SDA_OUT_PA01)); 
  return true;
}
#endif
...
bool  i2c_start_pa01(uint8_t addr)
#if I2C_HARDWARE
{
  uint8_t   twst;
#if I2C_TIMEOUT
...
bool  i2c_rep_start_pa01(uint8_t addr)
#if I2C_HARDWARE
{
  return i2c_start_pa01(addr);
}
#else
{
  __asm__ __volatile__ 
...
bool  i2c_start_wait_pa01(uint8_t addr)
#if I2C_HARDWARE
{
  uint8_t   twst;
...
#else
{
 __asm__ __volatile__ 
   (
    ...
    " ret "
    : : [SDADDR] "I"  (SDA_DDR_PA01), [SDAPIN] "I" (SDA_PIN_PA01), [SDAOUT] "I" (SDA_OUT_PA01),
      [SCLIN] "I" (SCL_IN_PA01), [SCLPIN] "I" (SCL_PIN_PA01),
...
#endif
     : : [SCLDDR] "I"  (SCL_DDR_PA01), [SCLPIN] "I" (SCL_PIN_PA01), [SCLIN] "I" (SCL_IN_PA01),
       [SDAOUT] "I" (SDA_OUT_PA01), [SCLOUT] "I" (SCL_OUT_PA01),
       [SDADDR] "I"  (SDA_DDR_PA01), [SDAPIN] "I" (SDA_PIN_PA01)); 
}
#endif
...
bool i2c_write_pa01(uint8_t value)
#if I2C_HARDWARE
{	
  uint8_t   twst;
#if I2C_TIMEOUT
...
#else
{
  __asm__ __volatile__ 
    (
     ...
     " breq     _Li2c_get_ack_pa01           ;jump if TXreg is empty;; +1 = 2C \n\t"
#if I2C_PULLUP
     " cbi      %[SCLOUT],%[SCLPIN]     ;disable SCL pull-up \n\t"
#endif
     ...
     " nop \n\t"
     " nop \n\t"
     " brcc     _Li2c_write_low_pa01                                ;;+1/+2=5/6C\n\t"
     " nop                                                     ;; +1 = 7C \n\t"
     ...
     ::
      [SCLDDR] "I"  (SCL_DDR_PA01), [SCLPIN] "I" (SCL_PIN_PA01), [SCLIN] "I" (SCL_IN_PA01),
      [SDAOUT] "I" (SDA_OUT_PA01), [SCLOUT] "I" (SCL_OUT_PA01),
      [SDADDR] "I"  (SDA_DDR_PA01), [SDAPIN] "I" (SDA_PIN_PA01), [SDAIN] "I" (SDA_IN_PA01)); 
  return true; // fooling the compiler
}
#endif

uint8_t i2c_read_pa01(bool last)
#if I2C_HARDWARE
{
#if I2C_TIMEOUT
  uint32_t start = millis();
...
#else
{
  __asm__ __volatile__ 
    (
     ...
     " brcc	_Li2c_read_bit_pa01	        ;while receiv reg not full \n\t"
     ...
     " sbi	%[SCLDDR],%[SCLPIN]	;force SCL low         ;; 2C \n\t" 
     " cpi	r24,0                                          ;; 3C \n\t" 
     " breq	_Li2c_put_ack_low_pa01	;if (ack=0) ;; 5C \n\t" 
     " cbi	%[SDADDR],%[SDAPIN]	;release SDA \n\t" 
#if I2C_PULLUP
     " sbi      %[SDAOUT],%[SDAPIN]     ;enable SDA pull-up \n\t"
#endif
