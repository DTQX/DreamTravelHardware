...
bool __attribute__ ((noinline)) i2c_init_pa67(void) __attribute__ ((used));

// Start transfer function: <addr> is the 8-bit I2C address (including the R/W
// bit). 
// Return: true if the slave replies with an "acknowledge", false otherwise
bool __attribute__ ((noinline)) i2c_start_pa67(uint8_t addr) __attribute__ ((used)); 

// Similar to start function, but wait for an ACK! Will timeout if I2C_MAXWAIT > 0.
bool  __attribute__ ((noinline)) i2c_start_wait_pa67(uint8_t addr) __attribute__ ((used));
...
// stop condition.
// Return: true if the slave replies with an "acknowledge", false otherwise
bool __attribute__ ((noinline)) i2c_rep_start_pa67(uint8_t addr) __attribute__ ((used));

// Issue a stop condition, freeing the bus.
void __attribute__ ((noinline)) i2c_stop_pa67(void) asm("ass_i2c_stop_pa67") __attribute__ ((used));

// Write one byte to the slave chip that had been addressed
// by the previous start call. <value> is the byte to be sent.
// Return: true if the slave replies with an "acknowledge", false otherwise
bool __attribute__ ((noinline)) i2c_write_pa67(uint8_t value) asm("ass_i2c_write_pa67") __attribute__ ((used));


// Read one byte. If <last> is true, we send a NAK after having received 
// the byte in order to terminate the read sequence. 
uint8_t __attribute__ ((noinline)) i2c_read_pa67(bool last) __attribute__ ((used));

#if !I2C_HARDWARE
// map the IO register back into the IO address space
#define SDA_DDR_PA67       	(_SFR_IO_ADDR(SDA_PORT_PA67) - 1)
#define SCL_DDR_PA67       	(_SFR_IO_ADDR(SCL_PORT_PA67) - 1)
#define SDA_OUT_PA67       	_SFR_IO_ADDR(SDA_PORT_PA67)
#define SCL_OUT_PA67       	_SFR_IO_ADDR(SCL_PORT_PA67)
#define SDA_IN_PA67		(_SFR_IO_ADDR(SDA_PORT_PA67) - 2)
#define SCL_IN_PA67		(_SFR_IO_ADDR(SCL_PORT_PA67) - 2)
...
void __attribute__ ((noinline)) i2c_delay_half_pa67(void) asm("ass_i2c_delay_half_pa67")  __attribute__ ((used));
void __attribute__ ((noinline)) i2c_wait_scl_high_pa67(void) asm("ass_i2c_wait_scl_high_pa67")  __attribute__ ((used));

void  i2c_delay_half_pa67(void)
{ // function call 3 cycles => 3C
  ...
  __asm__ __volatile__ 
    (
     ...
     " brne _Lidelay_pa67                                           ;;5C+(x-1)2C+xC\n\t"
...
void i2c_wait_scl_high_pa67(void)
{
#if I2C_TIMEOUT <= 0
  __asm__ __volatile__ 
    ("_Li2c_wait_stretch_pa67: \n\t"
     " sbis	%[SCLIN],%[SCLPIN]	;wait for SCL high \n\t" 
     " rjmp	_Li2c_wait_stretch_pa67 \n\t"
     " cln                              ;signal: no timeout \n\t"
     " ret "
     : : [SCLIN] "I" (SCL_IN_PA67), [SCLPIN] "I" (SCL_PIN_PA67));
#else
  __asm__ __volatile__ 
...
#else
{
  __asm__ __volatile__ 
    (" cbi      %[SDADDR],%[SDAPIN]     ;release SDA \n\t" 
     ...
     " ret "
     : :
       [SCLDDR] "I"  (SCL_DDR_PA67), [SCLPIN] "I" (SCL_PIN_PA67), 
       [SCLIN] "I" (SCL_IN_PA67), [SCLOUT] "I" (SCL_OUT_PA67),
       [SDADDR] "I"  (SDA_DDR_PA67), [SDAPIN] "I" (SDA_PIN_PA67), 
       [SDAIN] "I" (SDA_IN_PA67), [SDAOUT] "I" (SDA_OUT_PA67)); 
  return true;
}
#endif
...
bool  i2c_start_pa67(uint8_t addr)
#if I2C_HARDWARE
{
  uint8_t   twst;
#if I2C_TIMEOUT
...
#else
{
  __asm__ __volatile__ 
    (
     ...
     " rcall    ass_i2c_wait_scl_high_pa67   ;wait until SCL=H\n\t"
     // 不等待，scl为低直接返回
     // " clr      r24                     ;set return value to false \n\t"
     // " sbis     %[SCLIN],%[SCLPIN]      ;check for SCL high \n\t"
     // " ret                              ;if low return with false \n\t" 
     ...
     " rcall    ass_i2c_delay_half_pa67      ;wait T/2 \n\t"
     " rcall    ass_i2c_write_pa67           ;now write address \n\t"
     " ret"
     : : [SDADDR] "I"  (SDA_DDR_PA67), [SDAPIN] "I" (SDA_PIN_PA67),
       [SDAOUT] "I" (SDA_OUT_PA67), [SCLOUT] "I" (SCL_OUT_PA67),
       [SCLIN] "I" (SCL_IN_PA67),[SCLPIN] "I" (SCL_PIN_PA67)); 
  return true; // we never return here!
}
#endif

bool  i2c_rep_start_pa67(uint8_t addr)
#if I2C_HARDWARE
{
  return i2c_start_pa67(addr);
}
#else
{
  __asm__ __volatile__ 
    ...
    (
     ...
     " rcall 	ass_i2c_delay_half_pa67	;delay  T/2 \n\t" 
     " cbi	%[SDADDR],%[SDAPIN]	;release SDA \n\t"
#if I2C_PULLUP
     " sbi      %[SDAOUT],%[SDAPIN]     ;enable SDA pull-up \n\t"
#endif
     ...
     " sbi 	%[SDADDR],%[SDAPIN]	;force SDA low \n\t"
     " rcall 	ass_i2c_delay_half_pa67	;delay	T/2 \n\t" 
     " rcall    ass_i2c_write_pa67       \n\t"
     " ret"
     : : [SCLDDR] "I"  (SCL_DDR_PA67), [SCLPIN] "I" (SCL_PIN_PA67),
       [SCLIN] "I" (SCL_IN_PA67), [SCLOUT] "I" (SCL_OUT_PA67), [SDAOUT] "I" (SDA_OUT_PA67),
       [SDADDR] "I"  (SDA_DDR_PA67), [SDAPIN] "I" (SDA_PIN_PA67)); 
  return true; // just to fool the compiler
}
#endif

bool  i2c_start_wait_pa67(uint8_t addr)
#if I2C_HARDWARE
{
  uint8_t   twst;
...
#else
{
 __asm__ __volatile__ 
   (
    ...
    " sbi 	%[SDADDR],%[SDAPIN]	;force SDA low \n\t" 
    " rcall 	ass_i2c_delay_half_pa67	;delay T/2 \n\t" 
    " rcall 	ass_i2c_write_pa67	        ;write address \n\t" 
    " tst	r24		        ;if device not busy -> done \n\t" 
    " brne	_Li2c_start_wait_done_pa67 \n\t" 
    " rcall	ass_i2c_stop_pa67	        ;terminate write & enable IRQ \n\t"
#if I2C_MAXWAIT
    " sbiw      r30,1                   ;decrement max wait counter\n\t"
    " breq       _Li2c_start_wait_done_pa67  ;if zero reached, exit with false -> r24 already zero!\n\t"
#endif
    " rjmp	_Li2c_start_wait1_pa67	;device busy, poll ack again \n\t" 
    "_Li2c_start_wait_done_pa67: \n\t"
    " clr       r25                     ;clear high byte of return value\n\t"
    " pop       __tmp_reg__             ;pop off orig argument \n\t"
    " ret "
    : : [SDADDR] "I"  (SDA_DDR_PA67), [SDAPIN] "I" (SDA_PIN_PA67), [SDAOUT] "I" (SDA_OUT_PA67),
      [SCLIN] "I" (SCL_IN_PA67), [SCLPIN] "I" (SCL_PIN_PA67),
...
#else
{
  __asm__ __volatile__ 
    (
...
#endif     
     " sbi      %[SDADDR],%[SDAPIN]     ;force SDA low \n\t" 
     " rcall    ass_i2c_delay_half_pa67      ;T/2 delay \n\t"
     " cbi      %[SCLDDR],%[SCLPIN]     ;release SCL \n\t" 
#if I2C_PULLUP
     " sbi      %[SCLOUT],%[SCLPIN]	;enable SCL pull-up \n\t"
#endif
     ...
     " sbi      %[SDAOUT],%[SDAPIN]     ;enable SDA pull-up \n\t"
#endif
     " rcall    ass_i2c_delay_half_pa67 \n\t"
#if I2C_NOINTERRUPT
     " sei                              ;enable interrupts again!\n\t"
#endif
     : : [SCLDDR] "I"  (SCL_DDR_PA67), [SCLPIN] "I" (SCL_PIN_PA67), [SCLIN] "I" (SCL_IN_PA67),
       [SDAOUT] "I" (SDA_OUT_PA67), [SCLOUT] "I" (SCL_OUT_PA67),
       [SDADDR] "I"  (SDA_DDR_PA67), [SDAPIN] "I" (SDA_PIN_PA67)); 
}
#endif
...
bool i2c_write_pa67(uint8_t value)
#if I2C_HARDWARE
{	
  uint8_t   twst;
#if I2C_TIMEOUT
...
#else
{
  __asm__ __volatile__ 
    (
     ...
     " rjmp _Li2c_write_first_pa67 \n\t"
     "_Li2c_write_bit_pa67:\n\t"
     " lsl      r24                     ;left shift into carry ;; 1C\n\t"
     "_Li2c_write_first_pa67:\n\t"
     " breq     _Li2c_get_ack_pa67           ;jump if TXreg is empty;; +1 = 2C \n\t"
#if I2C_PULLUP
     " cbi      %[SCLOUT],%[SCLPIN]     ;disable SCL pull-up \n\t"
#endif
     ...
     " nop \n\t"
     " nop \n\t"
     " brcc     _Li2c_write_low_pa67                                ;;+1/+2=5/6C\n\t"
     " nop                                                     ;; +1 = 7C \n\t"
     " cbi %[SDADDR],%[SDAPIN]	        ;release SDA           ;; +2 = 9C \n\t"
#if I2C_PULLUP
     " sbi      %[SDAOUT],%[SDAPIN]     ;enable SDA pull-up \n\t"
#endif
     " rjmp      _Li2c_write_high_pa67                              ;; +2 = 11C \n\t"
     "_Li2c_write_low_pa67: \n\t"
#if I2C_PULLUP
     " cbi      %[SDAOUT],%[SDAPIN]     ;disable pull-up \n\t"
#endif
     " sbi	%[SDADDR],%[SDAPIN]	;force SDA low         ;; +2 = 9C \n\t"
     " rjmp	_Li2c_write_high_pa67                               ;;+2 = 11C \n\t"
     "_Li2c_write_high_pa67: \n\t"
#if I2C_DELAY_COUNTER >= 1
     " rcall 	ass_i2c_delay_half_pa67	;delay T/2             ;;+X = 11C+X\n\t"
#endif
     ...
     " rcall    ass_i2c_wait_scl_high_pa67 \n\t"
     " brpl     _Ldelay_scl_high_pa67                              ;;+2 = 18C+X\n\t"
     "_Li2c_write_return_false_pa67: \n\t"
     " clr      r24                     ; return false because of timeout \n\t"
     " rjmp     _Li2c_write_return_pa67 \n\t"
     "_Ldelay_scl_high_pa67: \n\t"
#if I2C_DELAY_COUNTER >= 1
     " rcall	ass_i2c_delay_half_pa67	;delay T/2             ;;+X= 18C+2X\n\t"
#endif
     " rjmp	_Li2c_write_bit_pa67 \n\t"
...
#endif
#if I2C_DELAY_COUNTER >= 1
     " rcall	ass_i2c_delay_half_pa67	;delay T/2 ;; +X = 7C+X \n\t"
#endif
     " clr	r25                                            ;; 17C+2X \n\t"
     ...
     " rcall    ass_i2c_wait_scl_high_pa67 \n\t"
     " brmi     _Li2c_write_return_false_pa67                       ;; 13C + X \n\t "
     " sbis	%[SDAIN],%[SDAPIN]      ;if SDA hi -> return 0 ;; 15C + X \n\t"
     " ldi	r24,1                   ;return true           ;; 16C + X \n\t"
#if I2C_DELAY_COUNTER >= 1
     " rcall	ass_i2c_delay_half_pa67	;delay T/2             ;; 16C + 2X \n\t"
#endif
     "_Li2c_write_return_pa67: \n\t"
     " nop \n\t "
     " nop \n\t "
#if I2C_PULLUP
     " cbi      %[SCLOUT],%[SCLPIN]     ;disable SCL pull-up \n\t"
     ...
     ::
      [SCLDDR] "I"  (SCL_DDR_PA67), [SCLPIN] "I" (SCL_PIN_PA67), [SCLIN] "I" (SCL_IN_PA67),
      [SDAOUT] "I" (SDA_OUT_PA67), [SCLOUT] "I" (SCL_OUT_PA67),
      [SDADDR] "I"  (SDA_DDR_PA67), [SDAPIN] "I" (SDA_PIN_PA67), [SDAIN] "I" (SDA_IN_PA67)); 
  return true; // fooling the compiler
}
#endif

uint8_t i2c_read_pa67(bool last)
#if I2C_HARDWARE
{
#if I2C_TIMEOUT
  uint32_t start = millis();
...
#else
{
  __asm__ __volatile__ 
    (
     ...
     " nop \n\t"
#if I2C_DELAY_COUNTER >= 1
     " rcall	ass_i2c_delay_half_pa67	;delay T/2             ;; 4C+X \n\t" 
#endif
...
#endif
#if I2C_DELAY_COUNTER >= 1
     " rcall	ass_i2c_delay_half_pa67	;delay T/2             ;; 6C + 2X \n\t" 
#endif
     ...
     " sbi	%[SCLDDR],%[SCLPIN]	;force SCL low         ;; 2C \n\t" 
     " cpi	r24,0                                          ;; 3C \n\t" 
     " breq	_Li2c_put_ack_low_pa67	;if (ack=0) ;; 5C \n\t" 
     " cbi	%[SDADDR],%[SDAPIN]	;release SDA \n\t" 
#if I2C_PULLUP
     " sbi      %[SDAOUT],%[SDAPIN]     ;enable SDA pull-up \n\t"
#endif
     " rjmp	_Li2c_put_ack_high_pa67 \n\t" 
     "_Li2c_put_ack_low_pa67:                ;else \n\t" 
#if I2C_PULLUP
     " cbi      %[SDAOUT],%[SDAPIN]     ;disable pull-up \n\t"
#endif
     " sbi	%[SDADDR],%[SDAPIN]	;force SDA low         ;; 7C \n\t" 
     "_Li2c_put_ack_high_pa67: \n\t" 
     " nop \n\t "
     " nop \n\t "
     " nop \n\t "
#if I2C_DELAY_COUNTER >= 1
     " rcall	ass_i2c_delay_half_pa67	;delay T/2             ;; 7C + X \n\t" 
#endif
     ...
     " rcall    ass_i2c_wait_scl_high_pa67 \n\t"
#if I2C_DELAY_COUNTER >= 1
     " rcall	ass_i2c_delay_half_pa67	;delay T/2             ;; 11C + 2X\n\t" 
#endif
     "_Li2c_read_return_pa67: \n\t"
     " nop \n\t "
     " nop \n\t "
#if I2C_PULLUP
     " cbi      %[SCLOUT],%[SCLPIN]     ;disable SCL pull-up \n\t"
